...
void LinkedCellsContainer::applyPairwiseForces(const std::vector<std::shared_ptr<PairwiseForceSource>> &force_sources)
{
    ...
    for (Cell *cell : occupied_cells_references)
    {
        for (auto it1 = cell->getParticleReferences().begin(); it1 != cell->getParticleReferences().end(); ++it1)
        {
            ...
            for (Cell *neighbour : cell->getDiagNeighbourReferences())
            {
                if (cell->getAlreadyInfluencedBy().contains(neighbour))
                    continue;

                for (Particle *neighbour_particle : neighbour->getParticleReferences())
                {
                    if (ArrayUtils::L2Norm(p->getX() - neighbour_particle->getX()) > cutoff_radius)
                        continue;

                    for (const auto &force_source : force_sources)
                    {
                        std::array<double, 3> force;
                        if (force_source->operator std::string() == "Harmonic")
                        {
                            force = std::dynamic_pointer_cast<HarmonicPotential>(force_source)->calculateForce(*p, *neighbour_particle, true);
                        }
                        else
                        {
                            force = force_source->calculateForce(*p, *neighbour_particle);
                        }
                        p->setF(p->getF() + force);
            ...
            for (Cell *neighbour : cell->getOrthNeighbourReferences())
            {
                ...
                for (Particle *neighbour_particle : neighbour->getParticleReferences())
                {
                    ...
                    for (const auto &force_source : force_sources)
                    {
                        ...
                        if (force_source->operator std::string() == "Harmonic")
                        {
                            force = std::dynamic_pointer_cast<HarmonicPotential>(force_source)->calculateForce(*p, *neighbour_particle, false);
                        }
                        else
                        {
                            force = force_source->calculateForce(*p, *neighbour_particle);
                ...
                for (int dx = -1; dx < 2; ++dx)
                {
                    for (int dy = -1; dy < 2; ++dy)
                    {
                        for (int dz = -1; dz < 2; ++dz)
                        {
                            ...
                            // Add the neighbour to the current cells diagonal or orthagonal neighbour references
                            Cell &curr_neighbour = cells.at(cell_index);
                            if (dx == 0 || dy == 0 || dz == 0)
                            {
                                cell.addOrthNeighbourReference(&curr_neighbour);
                            }
                            else
                            {
                                cell.addDiagNeighbourReference(&curr_neighbour);
                            }
                        }
                    }
                }
