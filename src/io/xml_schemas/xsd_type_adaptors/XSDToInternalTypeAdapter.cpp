...
CuboidSpawner XSDToInternalTypeAdapter::convertToCuboidSpawner(const CuboidSpawnerType &cuboid, bool third_dimension)
{
    ...
    auto is_membrane = cuboid.is_membrane();
    ...
    if (is_membrane && !(grid_dimensions[0] == 1 || grid_dimensions[1] == 1 || grid_dimensions[2] == 1))
    {
        Logger::logger->error("Cuboid grid dimensions must be 1 in at least one direction if membrane is enabled");
        throw std::runtime_error("Cuboid grid dimensions must be 1 in at least one direction if membrane is enabled");
    }
    ...
    return CuboidSpawner{
        lower_left_front_corner, grid_dimensions, is_membrane, grid_spacing, mass, initial_velocity, static_cast<int>(type), epsilon, sigma,
...
CuboidSpawner XSDToInternalTypeAdapter::convertToSingleParticleSpawner(const SingleParticleSpawnerType &particle, bool third_dimension)
{
    ...
    return CuboidSpawner{
        position, {1, 1, 1}, false, 0, mass, initial_velocity, static_cast<int>(type), epsilon, sigma, third_dimension, particle.temperature()};
}
...
XSDToInternalTypeAdapter::convertToForces(const ForcesType &forces)
{
    ...
    if(forces.SmoothLJ()){
       ...
       double cutoff = forces.SmoothLJ()->cutOffR();
